...
var DRAW_OFFSET = 10 // how much draw offset in slot display from top
...
function SlotGame() {
	...
	function _changeImg(){
	    selected_id = document.getElementById('options').selectOptions.selectedIndex;
	    document.getElementById("logo").src = items[selected_id].id;
	}
	...
	_fill_canvas( game.c1, game.items1 );
	game.items2 = copyArray(items);
	shuffleArray(game.items2);
	_fill_canvas( game.c2, game.items2 );
	game.items3 = copyArray(items);
	shuffleArray(game.items3);
	_fill_canvas( game.c3, game.items3 );
	game.resetOffset =  (ITEM_COUNT ) * SLOT_HEIGHT;
	game.loop();
    });

    document.getElementById('options').onchange=function(){
		selected_id = document.getElementById('options').selectedIndex;
	    document.getElementById("logo").src = 'img/'+items[selected_id].id + '.png';
	}
	document.getElementById('start').onclick=function(){
		//start game on play button click
		// document.getElementById('play').style.background = "url(img/BTN_Spin_d.png) no-repeat";
		this.style.background = "url(img/BTN_Spin_d.png) no-repeat";
		game.restart();
    };
...
function Game() {
    ...
    this.c3 = document.getElementById('canvas3');
   
    //canvas style
    console.log(IMAGE_HEIGHT * ITEM_COUNT * 2);
    this.c1.style.height = IMAGE_HEIGHT * ITEM_COUNT * 2;
    this.c2.style.height = IMAGE_HEIGHT * ITEM_COUNT * 2;
    this.c3.style.height = IMAGE_HEIGHT * ITEM_COUNT * 2;
...
Game.prototype.update = function() {
	...
	document.getElementById('play').style.background = "url(img/BTN_Spin.png) no-repeat";
	// document.getElementById('results').style.display="block";
	...
	//     ec++;
	// }
	// document.getElementById('multiplier').text(ec);

	// document.getElementById('status').text(BLURB_TBL[ec]);
...
Game.prototype.draw = function( force ) {
	...
	if (this[stopped] || this[speedp] || force) {
		...
		if (this[offsetp] + DRAW_OFFSET > 0) {
		    // reset back to beginning
		    this[offsetp] = -this.resetOffset + SLOT_HEIGHT * 2 - DRAW_OFFSET;
		}
	    }

	    transform = this.trnOpen + '0px, '+(this[offsetp] + DRAW_OFFSET)+'px' + this.trnClose;
		this[cp].style.MozTransform = transform;
		this[cp].style.WebkitTransform = transform;
		this[cp].style.OTransform = transform;
	}
    }
}
